...
** Last update Sun Apr 12 18:25:28 2015 Manuel Trambert
*/
...
typedef struct		s_dsm_func
{
  int			type;
  char			*name;
  int			loop;
  int			(*type_fct)(unsigned char *str, int *i, int loop, int fd);
}			t_dsm_func;
...
int		get_size_oct(unsigned char *str, int *i, int loop, int fd);
int		get_size_ldi(unsigned char *str, int *i, int loop, int fd);
int		get_size_sti(unsigned char *str, int *i, int loop, int fd);
int		print_dir(unsigned char *str, int *i, int loop, int fd);
int		print_ind(unsigned char *str, int *i, int loop, int fd);
int		litle_endian();
...
/* parsing.c */
int	check_syntax(char **board, int line, char **text);

/* free.c */
void	free_end(t_tmp *tmp, int fd, int dest, t_champ *champ);
void	free_tab(char **board);
...
/* check_arg.c */
int	check_mul_arg(char **board, int line, char **text, int i);
