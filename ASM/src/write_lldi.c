...
** Last update Tue Apr  7 22:38:50 2015 Kévin Julien
*/
...
void	check_codlldi(int dest, char **param)
{
  char	cod;
  char	lldi;
  int	i;
  ...
  write(dest, &lldi, sizeof(char));
  write(dest, &cod, sizeof(char));
}

void	check_labellldi(int dest, char *str, int *octet, t_champ *champ)
{
  int	ind;
...
void	check_paramlldi(int dest, char *str, int *octet, t_champ *champ)
{
  short	dir;
  char	reg;
  ...
  if (str[0] == 'r')
    ...
    check_labellldi(dest, str, octet, champ);
  else
    {
...
void	write_lldi(int dest, char **tmp, int *octet, t_champ *champ)
{
  ...
  i = 0;
  check_codlldi(dest, param);
  while (param[i] != NULL)
    {
      check_paramlldi(dest, param[i], octet, champ);
      i++;
    }
}
