...
** Last update Thu Apr  9 10:10:02 2015 Kévin Julien
*/
...
void	check_codxor(int dest, char **param, int *octet)
{
  char	cod;
  char	and;
  int	i;
...
void	check_labelxor(int dest, char *str, int *octet, t_champ *champ)
{
  int	ind;
...
void	check_paramxor(int dest, char *str, int *octet, t_champ *champ)
{
  short	dir;
  char	reg;
  ...
  if (str[0] == 'r')
    ...
    check_labelxor(dest, str, octet, champ);
  else
    {
...
void	write_xor(int dest, char **tmp, int *octet, t_champ *champ)
{
  ...
  i = 0;
  check_codxor(dest, param, octet);
  while (param[i] != NULL)
    {
      check_paramxor(dest, param[i], octet, champ);
      i++;
    }
}
